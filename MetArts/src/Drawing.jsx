...
const ToolBarSelector = () => {
  ...
  const [prevStateX, setPrevStateX] =useState(null);
  const [prevStateY, setPrevStateY] =useState(null);
  ...
  function clearingFunction(){
    ...
    ctx.clearRect(0, 0, canvas.width, canvas.height); 
          setSavedCanvas(null); setSnapShot(null); 
  ...
  useEffect(() => {
    ...
    const setCanvasSize = () => {
      ...
      if (clearCanva) 
        ...
        if (saveImage) {
          ...
          const newContext = canvas.getContext("2d", { preserveDrawingBuffer: true }); 
          const link = document.createElement("a");
          link.download = `${Date.now()}.png`;
          ...
          originalContext.drawImage(newContext.canvas, 0, 0); 
    ...
    function drawRectangle(event){
      ctx.putImageData(snapshot, 0, 0);
      setPrevStateX(event.offsetX);
      setPrevStateY(event.offsetY);
      
      if (!isFillSet) 
        return ctx.strokeRect(event.offsetX,event.offsetY, prevStateX - event.offsetX, prevStateY - event.offsetY); 
      else ctx.fillRect(event.offsetX,event.offsetY, prevStateX - event.offsetX, prevStateY - event.offsetY); 
    }

    function drawCircle(event){
      ctx.beginPath();
      ctx.putImageData(snapshot, 0, 0);
      let radius = Math.sqrt(Math.pow((prevStateX-event.offsetX),2)+ Math.pow((prevStateY-event.offsetY),2));
      ctx.arc(prevStateX,prevStateY,radius,0,2*Math.PI);
      isFillSet ? ctx.fill(): ctx.stroke();
    }

    function drawTriangle(event){
      ctx.beginPath();
      ctx.putImageData(snapshot,0,0);
      ctx.moveTo(prevStateX,prevStateY);
      ctx.lineTo(event.offsetX,event.offsetY);
      ctx.lineTo(prevStateX*2- event.offsetX, event.offsetY);
      ctx.closePath();
      isFillSet? ctx.fill() : ctx.stroke();
    }
    ...
    const drawing = (event) => {
      ...
      if(selected.tool==="Brush"){
       ...
       drawRectangle(event);
       console.log("Rectangle invoked");
       
      }
      else if(selected.tool==="Circle"){
        drawCircle(event);
      }
      ...
      else if(selected.tool==="Eraser"){
        ctx.strokeStyle = "#fff";
        ctx.lineWidth = selected.size * 4;
        ctx.lineTo(event.offsetX, event.offsetY);
        ctx.stroke();
      }
...
}, [savedCanvas,selected.tool,selected.size,selected.color, isFillSet, clearCanva, saveImage,selectedImage]);


const location = useLocation(); 
useEffect(() => { if (location.state && location.state.imageData) { setSavedCanvas(location.state.imageData); location.state.imageData=null; location.state=null} }, [location.state]);
...
<ToolBar 
  ...
  setFillProperty={setFillProperty} 
  isFillSet={isFillSet} 
  setFill={setFill}  
...
/>
      ...
      <SavedDrawColumn onImageSelect={setSelectedImageInvoker}
      refresh={refresh}></SavedDrawColumn>
